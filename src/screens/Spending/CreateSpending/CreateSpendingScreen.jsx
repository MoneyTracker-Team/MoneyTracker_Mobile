...
const CreateSpendingScreen = ({ navigation }) => {
  ...
  const createSpending = async () => {
    if (Object.keys(spendingType).length === 0 || moneySpend === 0) {
      Alert.alert('Thông tin không đúng', 'Vui lòng điền đầy đủ thông tin để tạo mới chi tiêu.', [
        {
          text: 'Ok',
          style: 'cancel',
        },
      ...
      Alert.alert('Xác nhận tạo mới', 'Bạn có chắc chắn muốn tạo mới chi tiêu này không?', [
        {
          text: 'Hủy',
          style: 'cancel',
        },
        {
          ...
          onPress: async () =>
            await fetch('${baseUrl}/spends/create', {
              ...
              .then((data) => {
                Alert.alert('Tạo mới chi tiêu thành công', '', [
                  {
                    text: 'Ok',
