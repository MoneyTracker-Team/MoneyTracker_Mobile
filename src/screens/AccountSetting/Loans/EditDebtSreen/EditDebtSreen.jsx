...
import styles from './editDebtSreen.styles';
...
const EditDebtScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    setMoneySpend(selectedDebt.moneySpend);
    setSelectedDay(selectedDebt.dateTime?.substring(0, 10));
    setTime(new Date(selectedDebt.dateTime));
    setNote(selectedDebt.note);
    setNoteContent(selectedDebt.note);
    setLocation(selectedDebt.location);
    setLocationContent(selectedDebt.location);
    setFriends(selectedDebt.debtor);
    setImage(selectedDebt.image);
  }, [selectedDebt]);
  ...
  const deleteSpending = async () => {
    Alert.alert('Xác nhận xóa', 'Bạn có chắc chắn muốn xóa phiếu nợ này không?', [
      {
        text: 'Hủy',
        style: 'cancel',
      },
        ...
        onPress: async () =>
          await fetch(`https://moneytrackerserver-production.up.railway.app/loans/delete/${debtId}`, {
            ...
            .then((data) => {
              ...
              Alert.alert('Xóa phiếu nợ thành công', '', [
                {
                  text: 'Ok',
                  onPress: () =>
  ...
  const updateSpending = async () => {
    if (moneySpend === 0 || moneySpend === '') {
      Alert.alert('Thông tin không đúng', 'Vui lòng điền đầy đủ thông tin để cập nhật phiếu nợ.', [
        {
          text: 'Ok',
          style: 'cancel',
        },
      ...
      if (imageBase64 === '') {
        updatedSpending = {
          ...
          location: location,
          image: imageBase64,
          debtorId: listFriendForUpdate[0],
          note,
        };
      } else {
        updatedSpending = {
      ...
      Alert.alert('Xác nhận cập nhật', 'Bạn có chắc chắn muốn cập nhật phiếu nợ này không?', [
        {
          text: 'Hủy',
          style: 'cancel',
        },
        {
          ...
          onPress: async () =>
            await fetch(`https://moneytrackerserver-production.up.railway.app/loans/update/${debtId}`, {
              ...
              .then((data) => {
                setRerender(!rerender);
                Alert.alert('Cập nhật Phiếu nợ thành công', '', [
                  {
                    text: 'Ok',
                    onPress: () =>
                ...
                {!friends ? (
                  ...
                  <ScrollView horizontal>
                    {friends.map((friend, index) => (
                      <View key={friend.index} style={styles.friend_item_container}>
